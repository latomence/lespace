<section class="flex flex-row justify-center py-16 gap-56 h-screen">
    <div class="flex flex-col justify-center">
        <h1 class="text-5xl font-bold">VOTRE ESPACE <br>D’APRENTISSAGE</h1>
        <p class="py-3">Apprenez ce que vous voulez quand vous vouler grâce à notre <br>communautée grandissante !</p>
        <div class="border border-[#DEDEDEDE] rounded-lg p-3 mt-3 w-full flex flex-row justify-between align-center">
            <input type="text" placeholder="Que voulez-vous apprendre ?" class="border-none w-60">
            <div class="bg-[#FF6363] p-1 rounded-lg text-center text-white text-sm w-40">
                Rechercher
            </div>
        </div>
    </div>
    <div class="bg-black flex justify-center w-96 h-96 rounded-lg mt-20"></div>
</section>

<section class="bg-black h-auto overflow-hidden">
    <div class="mt-5">
        <h2 class="text-white text-xl ml-10 mt-10">Voyagez à travers les mots</h2>
        <div class="py-10">
            <ul class="gap-5 flex flex-row flex-wrap w-full justify-center mx-auto -rotate-12">
                <li class="bg-white w-28 h-28 rounded-lg"></li>
                <li class="bg-emerald-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-yellow-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-pink-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-blue-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-red-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-green-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-purple-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-indigo-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-gray-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-white w-28 h-28 rounded-lg"></li>
                <li class="bg-emerald-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-yellow-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-pink-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-blue-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-red-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-green-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-purple-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-indigo-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-gray-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-white w-28 h-28 rounded-lg"></li>
                <li class="bg-emerald-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-yellow-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-pink-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-blue-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-red-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-green-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-purple-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-indigo-600 w-28 h-28 rounded-lg"></li>
                <li class="bg-gray-600 w-28 h-28 rounded-lg"></li>

            </ul>
        </div>
    </div>
</section>

<section class="py-20">
    <div class="flex flex-row justify-center mx-auto">
        <div class="flex flex-row gap-3 flex-wrap w-96 -rotate-12">
            <div class="bg-black w-40 h-48 rounded-lg"></div>
            <div class="bg-black w-40 h-48 rounded-lg"></div>
            <div class="bg-black w-40 h-48 rounded-lg"></div>
            <div class="bg-black w-40 h-48 rounded-lg"></div>
            <div class="bg-black w-40 h-48 rounded-lg"></div>
            <div class="bg-black w-40 h-48 rounded-lg"></div>
        </div>
        <div class="flex flex-col w-96 text-center">
            <h1 class="text-3xl font-bold">DES ENSEIGNANTS CERTIFIES ET ETOILES</h1>
            <p class="py-4">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Dolor sunt facere fuga fugit aliquid.</p>
            <button class="bg-[#FF6363] rounded-lg text-center text-white w-64 px-4 mx-auto mt-3">Voir plus d'enseignents</button>
        </div>
    </div>
</section>

<section>
    <div>
        <h1 class="text-4xl font-bold text-center">Trouver le prof qui vous correspond !</h1>
    </div>
    <div class="flex flex-row w-9/12 flex-wrap justify-center mx-auto py-10">
        <div class="flex flex-col flex-wrap justify-center mx-auto w-2/3">
            <h2 class="font-bold text-xl">1. Je m'inscrit</h2>
            <p class="w-auto py-5">Inscrivez-vous en quelques étapes simples : cliquez sur "Inscription", remplissez le formulaire, choisissez votre type de cours, sélectionnez une date et heure convenables, effectuez votre paiement sécurisé, et recevez une confirmation par e-mail.  Commencez dès maintenant votre parcours d'apprentissage en ligne !</p>
        </div>
        <div class="bg-black h-80 rounded-lg w-1/3"></div>
    </div>
    <div class="flex flex-row w-9/12 justify-center mx-auto py-10">
        <div class="bg-black h-80 rounded-lg w-1/3"></div>
        <div class="flex flex-col flex-wrap justify-center mx-auto w-2/3 ml-24">
            <h2 class="font-bold text-xl">2. Je profite des X-Pay</h2>
            <p class="w-auto py-5">L'achat de notre monnaie virtuelle est un processus simple et rapide. Pour commencer, cliquez sur le bouton "Acheter" situé sur notre page principale. Vous serez redirigé vers un formulaire où vous devrez fournir quelques informations de base, telles que votre nom, votre adresse e-mail et le montant de monnaie virtuelle  que vous souhaitez acheter.</p>
            <button class="bg-[#FF6363] rounded-lg text-center text-white w-64 px-4 mt-3">Voir les offres</button>
        </div>
    </div>
    <div class="flex flex-row w-9/12 flex-wrap justify-center mx-auto py-10">
        <div class="flex flex-col flex-wrap justify-center mx-auto w-2/3">
            <h2 class="font-bold text-xl">3. Je participe à un cours</h2>
            <p class="w-auto py-5">Rejoindre nos cours est un processus simple et rapide. Pour participer à l'un de nos cours passionnants, commencez par cliquer sur le bouton "Participer" sur notre page dédiée aux cours. Vous serez dirigé vers un formulaire d'inscription où vous devrez fournir quelques  informations personnelles, telles que votre nom, votre adresse e-mail et votre niveau d'expérience dans le domaine du cours.</p>
        </div>
        <div class="bg-black h-80 rounded-lg w-1/3"></div>
    </div>
</section>

<section class="p-5">
    <div class="flex flex-col ml-10 bg-[#D9D9D9] mx-10 p-5 py-10 rounded-lg">
        <h1 class="text-4xl font-bold">Inscrivez-vous dès maintenant !</h1>
        <p class="py-4">Inscrivez-vous dès maintenant et profiter de nos offres sur la monnaie x-pay et profiter de vos premier cours !</p>
        <button class="bg-[#FF6363] rounded-lg text-center text-white w-64 px-4 mt-3">Je m'inscrit</button>
    </div>
</section>

<script src="https://cdn.tailwindcss.com"></script>